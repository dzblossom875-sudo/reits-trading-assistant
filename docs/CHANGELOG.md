...
### 持仓列索引修复 + Dashboard图一终止日控件

#### Bug Fix：持仓xlsx列位置漂移导致读取"累计盈亏"列而非"本币持仓市值"
- **fix** (`src/data_loader.py`, `src/position_calculator.py`): 持仓文件改用列名映射替代位置索引
  - 旧xlsx（57列）列43=本币持仓市值；新xlsx（58列）多"原始权益人"列，列44=本币持仓市值
  - 旧代码 `{43: "market_value"}` 读到"累计盈亏(元)"（含大量负数），导致49条负持仓误报、3/25仓位计算为0%
  - 改为 `{"本币持仓市值(元)": "market_value"}` 按列名映射，彻底规避列位置漂移
  - `data_loader.py` 同步改为 `header=0` 读取，列名映射同步更新
  - 修复后：持仓26条→75条，最新仓位0%→95.02%

#### feat (`dashboard.py`): 图一策略跟踪新增终止日控件
- 侧边栏"图一：核心趋势"新增"终止日"date_input，默认值=数据最新日
- `df_p1` 改为双边截取 `[起始日, 终止日]`，支持查看任意历史区间
- 归一化锚点逻辑不变（始终从完整df查找基准日对应值）
- `uirevision` 加入终止日，确保切换日期时图表正确刷新
- 指标卡期间标签跟随实际数据范围显示
